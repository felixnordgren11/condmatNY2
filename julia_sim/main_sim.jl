...
function steepest_descent(positions, forces, nsteep, Csteep, ret_mv = false)
    f = forces
    pos = positions
    fmax = 0
...
=#
function Verlet2(positions, neighbors, velocity)
...
T_array, Etot_array, z0_positions, fmax= Verlet2(positions, neighbors, velocities)

avg_E = mean(Etot_array)
avg_T = mean(T_array)

println(avg_E, ", ", avg_T, ", ", Ekin)
...
plot(filtered_timesteps, filtered_T_array, title = "Temperature vs Time", xlabel = "Timestep", ylabel = "Temperature", legend = false, ylim = (100, 250))
...
plot(filtered_timesteps, filtered_Etot_array, title = "Total Energy vs Time", xlabel = "Timestep", ylabel = "Total Energy", color = :red, legend = false, ylim = (-440, -436))

# Save the total energy plot if needed
savefig("total_energy_vs_time_filtered.png")


# #Plot to see if steepest descent works
# nsteps = 1:nsteep
# # Plot the average position difference
# plot(nsteps, avg_diff, title = "Average Position Difference per Step", xlabel = "Step", ylabel = "Average Difference", legend = false)
# savefig("average_position_difference.png")
