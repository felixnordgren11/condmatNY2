...
function assgn_mom_sub_velocities(T_initial, random_file)
...
=#
function Verlet2(positions, neighbors, velocity, timestep, simulation_time)
    dt = timestep
...
T_array, Etot_array, z0_positions = Verlet2(positions, neighbors, velocities, dt, sim_t)
...
plot(filtered_timesteps, filtered_T_array, title = "Temperature vs Time", xlabel = "Timestep", ylabel = "Temperature", legend = false, ylim = (0, 400))
...
plot(filtered_timesteps, filtered_Etot_array, title = "Total Energy vs Time", xlabel = "Timestep", ylabel = "Total Energy", color = :red, legend = false, ylim = (-500, -400))
